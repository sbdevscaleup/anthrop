...
async function validateManualAdminSelection(data: CreatePropertyInput) {
...
export async function createPropertyForUser(
  userId: string,
  unsafeData: CreatePropertyInput,
) {
  const data = createPropertyInputSchema.safeParse(unsafeData);
  if (!data.success) {
    return {
      success: false,
  ...
  try {
    ...
    const [created] = await db
      ...
      .values({
        ...
        agentUserId: data.data.agentUserId ?? null,
    ...
    if (created.agentUserId) {
      const existingActiveAssignment = await db.query.propertyAgentAssignment.findFirst({
        where: and(
          eq(propertyAgentAssignment.propertyId, created.id),
          isNull(propertyAgentAssignment.endedAt),
        ),
      });

      if (!existingActiveAssignment) {
        await db.insert(propertyAgentAssignment).values({
          propertyId: created.id,
          organizationId: created.organizationId,
          agentUserId: created.agentUserId,
          assignedByUserId: userId,
          reason: "created_with_agent_assignment",
        });
      }
    }
    ...
    const attributeRows: Array<typeof propertyAttributeValue.$inferInsert> = [];
